...
router.post('/', [
  ...
  body('priority').optional().isIn(['VERY_URGENT', 'URGENT', 'IMPORTANT']).withMessage('Invalid priority'),
  body('deadline').optional().isISO8601().toDate().withMessage('Invalid deadline')
  ...
  try {
    ...
    const { title, description, priority = 'IMPORTANT', deadline } = req.body;
